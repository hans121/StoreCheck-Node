...
var productionLineHierarchy = db.db.collection('production-line-hierarchy');
var dbUtils = require('../../database/database-utils');
var schema = require('./../schema/schema');

dbUtils.addStandardMethods(exports, productionLineHierarchy);
...
function _readLine(lines, index, callback2) {
    ...
    if(lineItems.length > 25) {
        ...
        hierarchy_record.source = 'import';
        hierarchy_record.version = schema.currentVersion;

        productionLineHierarchy.insert(hierarchy_record, {safe: true}, function(err, object) {
            if(err != null) {
                winston.log('error', 'while reading production line file, identity_id=' + object.identity_id + ', explanation=' + err);
            }
            _readLine(lines, index + 1, callback2);
        });
    } else {
        winston.log('warn', 'while reading production line file, line ' + (index + 1) + ' (1-indexed) did not have enough records to parse');
        _readLine(lines, index + 1, callback2);
    }
}